...
/**
* Comparison of tables and fields with SQL result
*
* @author Gladisson <gladisson@gmail.com>
*
* Usage example:

	$dados_conexao1 = array(
		"host"=>"locahost",
		"banco"=>"*****",
		"usuario"=>"*****",
		"senha"=>"*****"
	...
	$compare = new compareMySQLDatabase();
	$sql = $compare->compare($dados_conexao1,$dados_conexao2);
*/

class compareMySQLDatabase {
	
    public function __construct(){}
    
	function conexao($dados){

		try{
			$PDO = new PDO( 'mysql:host='.$dados['host'].';dbname='.$dados['banco'].'', $dados['usuario'], $dados['senha'], 
	...
	function compare($dados_conexao1,$dados_conexao2){

		$PDO1 = $this->conexao($dados_conexao1);
		$PDO2 = $this->conexao($dados_conexao2);
		$sql  = " SELECT table_schema, table_name, table_type FROM information_schema.tables WHERE table_schema = '".$dados_conexao1['banco']."' ";
		$consulta = $PDO1->prepare($sql);
		$consulta->execute();
		$fetchAll = $consulta->fetchAll(PDO::FETCH_ASSOC);
		$dbStructUpdater = new dbStructUpdater();
		$retorno = "";
		foreach($fetchAll as  $row){
			...
			$create_table1 = $fetch['Create Table'];

			$sql  = " show create table ".$tabela." ";
			$consulta = $PDO2->prepare($sql);
			$consulta->execute();
			$fetch = $consulta->fetch(PDO::FETCH_ASSOC);
			$create_table2 = $fetch['Create Table'];

			$area = "";
			$dados = $dbStructUpdater->getUpdates($create_table2, $create_table1);
